...
// const rpc_url = "https://mainnet.infura.io/v3/8dc32d31130342d8a4b85e682c24926d"
// Ropsten Net
...
const rpc_url = "https://ropsten.infura.io/v3/8dc32d31130342d8a4b85e682c24926d"
...
jQuery(function($) {
    $(document).ready(function () {
        function load_js(){
            ...
            if ((address == undefined || address == null || address == "") && ($("#trade_workspace").length || $("#reward_body").length) || $("#account_body").length) {
            ...
            function goto_exchange() {
                window.location.href = 'exchange/TM-WETH';
            }

            if ($("#account_body").length) {
                $("#metamask_login.gotoBtn").click(goto_exchange);
                $("#create_tokenmom_wallet.gotoBtn").click(goto_exchange);
                $("#import_tokenmom_wallet.gotoBtn").click(goto_exchange);
                $("#import_json_wallet.gotoBtn").click(goto_exchange);

                $("#metamask_login.walletBtn").click(metamask_login);
                $("#create_tokenmom_wallet.walletBtn").click(create_tokenmom_wallet);
                $("#import_tokenmom_wallet.walletBtn").click(import_tokenmom_wallet);
                $("#import_json_wallet.walletBtn").click(import_json_wallet);
                $("#importBrowserWallet .trade-by-address").click(function () {
                    ...
                    var addr = keythereum.privateKeyToAddress(privkey.substring(2));
                        ...
                        if (ethers.utils.getJsonWalletAddress(json)) {
                            ...
                            ethers.Wallet.fromEncryptedJson(json, password, null).then(function(wallet) {
                                ...
                                if (error.message === 'invalid password') {
                                    ...
                                    let alert = "<span>Please enter correct password</span>";
                                    $('#json_password').parent().next().children().remove();
                                    $('#json_password').parent().next().html(alert);
                                    $("#importJsonWallet .text-danger").css("display", "block");
                                } else {
                                    console.log(error);
                                    console.log('Error Decrypting Wallet');
                                    let alert = "<span>Error Decrypting Wallet</span>";
                                    $('#json_password').parent().next().children().remove();
                                    $('#json_password').parent().next().html(alert);
                                    $("#importJsonWallet .text-danger").css("display", "block");
                                }
                            ...
                            let alert = "<span>Unknown JSON wallet format</span>";
                            $('#json_password').parent().next().children().remove();
                            $('#json_password').parent().next().html(alert);
                            $("#importJsonWallet .text-danger").css("display", "block");
                        }
                ...
                $("#enterNickName .trade-by-address").click(function () {
                    ...
                    if (login_method == 1) {
                        ...
                        $("#importJsonWallet .text-danger").css("display", "none");
                        $("#importJsonWallet .text-danger").text("");
                    }
            ...
            if (wallet.address != null && wallet.address != "") {
                ...
                web3js = new Web3(new Web3.providers.HttpProvider(rpc_url));
            ...
            update_trade_account();
        }

        function update_trade_account() {
       ...
       $(document).on('change paste keyup', "#json_password", function(){
            $("#importJsonWallet .text-danger").css("display", "none");
            $("#importJsonWallet .text-danger").text("");
        });
        ...
        function isValidPrivateKey(privateKeyIn) {
            ...
            return privateKey && "0x" === privateKey.substring(0, 2) && privateKey.length == 66;
        }
        function verifyPrivateKey(addr, privateKeyIn) {
            var privateKey = privateKeyIn;

            return privateKey && "0x" === privateKey.substring(0, 2) && addr.toLowerCase() === EthJS.Util.toChecksumAddress("0x" + EthJS.Util.privateToAddress(privateKey).toString("hex")).toLowerCase();
        }
        ...
        function set_wallet(addr, privkey, type) {
            if ("0x" === privkey.substring(0, 2))
                localStorage.setItem('wallet_privatekey', privkey.substring(2));
            else
        ...
        function wallet_login(nick_name, wallet_type) {
            if (currentWalletAddress != undefined && currentWalletAddress != null && currentWalletAddress != "") {
                ...
                if (wallet_type == 'META_MASK') {
                    ...
                    $('#metamask_login.walletBtn').css('bottom', "55px");
                    $('#metamask_login.gotoBtn').css('display', "block");
                    ...
                    $('#create_tokenmom_wallet.walletBtn').css('bottom', "55px");
                    $('#create_tokenmom_wallet.gotoBtn').css('display', "block");
                    ...
                    $('#import_tokenmom_wallet.walletBtn').css('bottom', "55px");
                    $('#import_tokenmom_wallet.gotoBtn').css('display', "block");
                    ...
                    $('#import_json_wallet.walletBtn').css('bottom', "55px");
                    $('#import_json_wallet.gotoBtn').css('display', "block");
        ...
        function metamask_disconnect() {
            ...
            $('#metamask_login.walletBtn').css('bottom', "25px");
            $('#metamask_login.gotoBtn').css('display', "none");
        ...
        function tokenmom_disconnect() {
            ...
            $('#create_tokenmom_wallet.walletBtn').css('bottom', "25px");
            $('#create_tokenmom_wallet.gotoBtn').css('display', "none");
        ...
        function privatekey_disconnect() {
            ...
            $('#import_tokenmom_wallet.walletBtn').css('bottom', "25px");
            $('#import_tokenmom_wallet.gotoBtn').css('display', "none");
        ...
        function json_disconnect() {
            ...
            $('#import_json_wallet.walletBtn').css('bottom', "25px");
            $('#import_json_wallet.gotoBtn').css('display', "none");
        ...
        function zeroex_login() {
            web3js.version.getNetwork((err, netId) => {
                ...
                if (web3Status == 1) {
                    ...
                    var wallet = get_wallet();
                    var privkey = wallet.privkey;
                    if ("0x" === privkey.substring(0, 2))
                        privkey = privkey.substring(2);
                    providerEngine.addProvider(new ZeroEx.PrivateKeyWalletSubprovider(privkey));
                }
                ...
                providerEngine.addProvider(new ZeroEx.RPCSubprovider(rpc_url));
